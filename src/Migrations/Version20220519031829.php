...
final class Version20220519031829 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__location AS SELECT id, parent_location_id, created_at, updated_at, deleted_at, label, hidden FROM location');
        ...
        $this->addSql('INSERT INTO location (id, parent_location_id, created_at, updated_at, deleted_at, label, hide_from_search) SELECT id, parent_location_id, created_at, updated_at, deleted_at, label, hidden FROM __temp__location');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('DROP TABLE location');
        $this->addSql('CREATE TABLE location (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, parent_location_id INTEGER DEFAULT NULL, label VARCHAR(255) NOT NULL, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, deleted_at DATETIME DEFAULT NULL, hidden BOOLEAN NOT NULL)');
        $this->addSql('INSERT INTO location (id, parent_location_id, label, hidden, created_at, updated_at, deleted_at) SELECT id, parent_location_id, label, hide_from_search, created_at, updated_at, deleted_at FROM __temp__location');
        $this->addSql('DROP TABLE __temp__location');
        $this->addSql('CREATE INDEX IDX_5E9E89CB6D6133FE ON location (parent_location_id)');
    }
}
